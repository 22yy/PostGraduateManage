<template>
  <div>
    ...
    <el-breadcrumb separator="/" style="margin-bottom:20px">
      ...
      <el-breadcrumb-item>查询课表</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  methods: {
    deleteSCT(row) {
      ...
      const that = this
      axios.post('http://175.24.188.37:10086/SCT/deleteBySCT', sct).then(function (resp) {
        if (resp.data === true) {
          that.$message({
            showClose: true,
            message: '退课成功',
  ...
  created() {
    ...
    const term = sessionStorage.getItem('currentTerm')
    const that = this
    axios.get('http://175.24.188.37:10086/SCT/findBySid/' + sid + '/' + term).then(function (resp) {
