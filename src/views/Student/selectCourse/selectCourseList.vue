...
export default {
  methods: {
    select(row) {
      ...
      const that = this;
      axios
        .post("http://175.24.188.37:10086/SCT/save", sct)
        .then(function (resp) {
          if (resp.data === "选课成功") {
            that.$message({
              showClose: true,
    ...
    deleteCourseTeacher(row) {
      const that = this;
      axios
        .post("http://175.24.188.37:10086/courseTeacher/deleteById", row)
        .then(function (resp) {
          if (resp.data === true) {
            that.$message({
              showClose: true,
  ...
  watch: {
    ruleForm: {
      handler(newRuleForm, oldRuleForm) {
        ...
        axios
          .post(
            "http://175.24.188.37:10086/courseTeacher/findCourseTeacherInfo",
            newRuleForm
          )
          .then(function (resp) {
